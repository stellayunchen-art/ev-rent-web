...
def _derive_boundary(std_low, std_high, bench_rows):
    ...
    ⚠️ 2026-07-28修复：find_benchmarks()改成"最近5个"之后（不再限制同城市），偏远地区
    容易出现"5个最近对标里没一个真的近"——实测肇庆四会某站点，最近的4个对标全在13-18km外
    的顺德/三水/南海，边界普遍550-750，直接把边界顶到行政区标准上限，跟本地实际情况脱节。
    用户对比领导的工具后发现对方把>20km的对标标"仅展示"、不参与边界计算，我们复用同一个
